...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.ravend.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getMiningInfo = function(callback) {
  this.node.services.ravend.getMiningInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
	var miningInfo = {
...
StatusController.prototype.getPeerInfo = function(callback) {
  this.node.services.ravend.getPeerInfo(function(err, response) {
    if (err) {
      return callback(err);
    }
  var peers = [];
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.ravend.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.ravend.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.ravend.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.ravend.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.ravend.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.ravend.height,
