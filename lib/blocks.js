...
BlockController.DEFAULT_BLOCK_CACHE_SIZE = 2000;
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.ravend.getBlockHeader(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.ravend.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new ravencore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in ravencore-node.
      var header = ravencore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [ravencore.Transaction().fromBufferReader(br)];

      self.node.services.ravend.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.ravend.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.ravend.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
