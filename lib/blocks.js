...
BlockController.prototype.getBlockByHash = function(hash, next) {
    ...
    if (blockCached) {
        ...
        , function (callback) {
            ...
            return self.node.services.ravend.getDetailedTransaction(txHash, function (err, trx) {
...
BlockController.prototype.transformBlock = function(block, info, reward, transaction) {
  ...
  var poolAddress;
      poolAddress = transaction.outputs[0].address;
  return {
    hash: block.hash,
  ...
  if (summaryCache) {
      ...
      if (err) {
        ...
        var summary = {
                ...
                poolInfo: self.getPoolInfo(transaction.outputs[0].address),
